	...
	for(;q--;){
		...
		if(op==1){
			...
			while(dep[l]>=dep[zx]){//include zx
				int lt=lst(l);
				if(dep[lt]>=dep[zx]){
					for(int i=0;i<min(22,dep[l]-dep[lt]);++i)vl[sl++]=1;
					vl[sl++]=a[lt];
					l=lt;l=fa[l];
					++cnt;
				}else{
					for(int i=0;i<min(22,dep[l]-dep[zx]);++i)vl[sl++]=1;
					vl[sl++]=a[zx];
					break;
				}
				if(cnt>12){
			...
			while(dep[r]>dep[zx]){
				int lt=lst(r);
				if(dep[lt]>dep[zx]){
					for(int i=0;i<min(22,dep[r]-dep[lt]);++i)vr[sr++]=1;
					vr[sr++]=a[lt];
					r=lt;r=fa[r];
					++cnt;
				}else{
					for(int i=0;i<min(22,dep[r]-dep[zx]);++i)vr[sr++]=1;
					break;
				}
				if(cnt>12){
					cout<<"0\n";
