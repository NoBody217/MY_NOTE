...
void Main(){
    ...
    if(m==1){
        int ni=-1;
        for(int i=1;i<=n;i++) cmax(ni,mx-a[i]);
        for(int _=1;_<=ni;_++){
            for(int i=2;i<=n+1;i++){
                if(i==n+1||a[i]==mx) ans+=(a[i-1]!=mx);
            }
            for(int i=1;i<=n;i++) a[i]=min(mx,a[i]+1);
        }
        cout<<ans<<endl;
    }else{
        ans=inf;
        ...
        for(int d=0;d<=k;d++){
            msp(dp);
            for(int j=0;j<=k;j++) dp[1][j][0]=dp[1][j][1]=j;
            for(int i=2;i<=n;i++){
                for(int j=0;j<=k;j++){
                    if(a[i]>0&&a[i]-j<=0){
                        cmin(dp[i][j-a[i]][0],dp[i-1][j][0]);
                        cmin(dp[i][j-a[i]][1],dp[i-1][j][1]);
                    }
                    if(a[i]<=0){
                        cmin(dp[i][j-a[i]][0],dp[i-1][j][0]-a[i]);
                        cmin(dp[i][j-a[i]][1],dp[i-1][j][1]-a[i]);
                    }
                    if(a[i]>d&&a[i]-j<=d){
                        cmin(dp[i][j-(a[i]-d)][0],dp[i-1][j][1]);
                    }
                    if(a[i]<=d){
                        cmin(dp[i][j-(a[i]-d)][0],dp[i-1][j][1]-(a[i]-d));
                    }
                    if(a[i]>-d&&a[i]-j<=-d){
                        cmin(dp[i][j-(a[i]+d)][1],dp[i-1][j][0]);
                    }
                    if(a[i]<=-d){
                        cmin(dp[i][j-(a[i]+d)][1],dp[i-1][j][0]-(a[i]+d));
                    }
                }
            }
            for(int j=0;j<=k;j++)
